...
def render_html(
    ...
    for source in sources:
        # Skip non-listing sources whose output is the directory index
        # (e.g., a package's ``__init__.py`` rendered as ``index.html``);
        # the directory's listing entry already represents that page.
        if source.output_path.name == "index" and not isinstance(
